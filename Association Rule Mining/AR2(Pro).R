...
dat2=data.frame(d2$reason,d2$higher,d2$studytime,AvgGrade=rowMeans(d2[,31:33]))
dat2
summary(dat2)
...
rules2=apriori(dat2,parameter = list(supp=0.01,conf=0.6))
inspect(rules2)
rules2.2=subset(rules2,subset=lift>2)
inspect(rules2.2)

#3.Sex-Age-StudyTime-Absence
dat3=data.frame(d2[,"sex"],d2[,"age"],d2[,"studytime"],d2[,"absences"])
...
#Sex-Age-AvgGrade
#dat4=data.frame(d2$sex,d2$age,AvgGrade=rowMeans(d2[,31:33]))
...
dat4=data.frame(d2$studytime,d2$internet,AvgGrade=rowMeans(d2[,31:33]))
...
dat5=data.frame(d2$Fedu,d2$Medu,d2$guardian)
...
#6.Sex-Age-Rel-Goout
dat6=data.frame(d2$sex,d2$age,d2$romantic,d2$freetime)
...
dat7=data.frame(d2$sex,d2$age,d2$health,Alcho=rowSums(d2[,27:28]))
summary(dat7)
...
#9.PEdu,AvgGrades
dat9=data.frame(d2$famrel,PEdu=rowSums(d2[,7:8]),AvgGrades=rowMeans(d2[,31:33]))
summary(dat9)
